...
function addSociete( $nom, $adresse,$logo,$ext_logo )
{
	...
	$this->Image($logo , 5 ,7, 60 , 25 , $ext_logo);
	$this->SetXY( $x1, $y1 );
...
// Client address
function addClientAdresse( $cliente,$direccion,$numerodoc,$email,$telefono )
{
	$r1     = $this->w - 180;
	$r2     = $r1 + 68;
	$y1     = 40;
	...
	$this->SetXY( $r1, $y1+15);
	$this->MultiCell( 150, 4, $numerodoc);
	$this->SetXY( $r1, $y1+20);
...
//                  "qte"     => qte,
//                  "tva"     => code_tva );
function addTVAs( $igv, $total,$monedas )
{
	$this->SetFont('Arial','',8);
	...
	$this->SetFont( "Arial", "", 8);
	$this->SetXY( $re, $y1+5 );
	$this->Cell( 17,4, $monedas.sprintf("%0.2F", $total-($total*$igv/($igv+100))), '', '', 'R');
	$this->SetXY( $re, $y1+10 );
	$this->Cell( 17,4, $monedas.sprintf("%0.2F", ($total*$igv/($igv+100))), '', '', 'R');
	$this->SetXY( $re, $y1+14.8 );
	$this->Cell( 17,4, $monedas.sprintf("%0.2F", $total), '', '', 'R');
